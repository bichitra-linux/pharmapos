...
export default function ReturnsIndex() {
    ...
    const { data: customerReturns, isLoading: loadingCustomer } = useQuery({
        ...
        queryFn: async () => {
            const res = await api.get('/customer-returns', { params: { page, per_page: 15 } });
            return res.data;
        },
    ...
    const customerColumns: Column<CustomerReturn>[] = [
        ...
        { key: 'reason', header: 'Reason' },
        {
            ...
            header: 'Status',
            render: (item) => (
                <Badge variant={item.status === 'completed' ? 'success' : item.status === 'rejected' ? 'destructive' : 'default'}>
                    {item.status}
                </Badge>
            ),
        },
    ...
    const supplierColumns: Column<SupplierReturn>[] = [
        { key: 'return_number', header: 'Return #', sortable: true },
        { key: 'purchase', header: 'Purchase #', render: (item) => item.purchase?.purchase_number || '-' },
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-1 rounded-lg bg-gray-100 p-1 w-fit">
                <button
                    ...
                    className={`rounded-md px-4 py-2 text-sm font-medium ${tab === 'customer' ? 'bg-white shadow' : 'text-gray-500'}`}
                >
                    Customer Returns
                </button>
                <button
                    onClick={() => { setTab('supplier'); setPage(1); }}
                    className={`rounded-md px-4 py-2 text-sm font-medium ${tab === 'supplier' ? 'bg-white shadow' : 'text-gray-500'}`}
                >
                    Supplier Returns
                </button>
            </div>
